...
void IRAM_ATTR onTimer(){
  ...
  pc->setValue( ("Temp:" + temperature));
  pc->notify();
  delay(100);
  pc->setValue(("Humd:" + humidity ));
  pc->notify();
  delay(100);
  pc->setValue(("LPG:" + mq2ppm));
  pc->notify();
  delay(100);
  pc->setValue(("CO:" + mq7ppm));
  pc->notify();
}
...
void loop(){
  ...
  */
  /*
  pc->setValue((temperature));
  pc->notify();
  delay(100);
  pc->setValue((humidity));
  pc->notify();
  delay(100);
  pc->setValue((mq2ppm));
  pc->notify();
  ...
  */
  showAlarm();
  /*pc->setValue( (temperature + "#" + humidity + "#" + mq2ppm + "#" + mq7ppm));
  pc->notify();*/
  delay(1000);
}
...
void calibrateSensors(){
  ...
  sensor_volt_mq2 = sensorValue_mq2/4096*5.0; //esp32 has 12 bit ADC
  RS_air_mq2 = (5.0-sensor_volt_mq2)/sensor_volt_mq2; // omit *RL
  sensorValue_mq7 = sensorValue_mq7/100.0;
  sensor_volt_mq7 = sensorValue_mq7/4096*5.0; //esp32 has 12 bit ADC
  RS_air_mq7 = (5.0-sensor_volt_mq7)/sensor_volt_mq7; // omit *RL
  
  R0_mq2 = RS_air_mq2/10.0; // The ratio of RS/R0 is 10 in a clear air (MQ2)
...
void readSensors(){
  ...
  sensor_volt_mq2=(float)mq2sensorValue/4096*5.0;
  RS_gas_mq2 = (5.0-sensor_volt_mq2)/sensor_volt_mq2; // omit *RL
  sensor_volt_mq7=(float)mq7sensorValue/4096*5.0;
